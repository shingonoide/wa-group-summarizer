...
async function performSummarization(n) {
  ...
  try {
    ...
    const shouldIncludeMedia = includeMediaCheckbox?.checked || includeMedia || false;
    ...
    console.log(`[Sidepanel] Got ${messages?.length || 0} messages`);

    const estimate = estimateTokens(messages);
    console.log(`[Sidepanel] Estimated tokens: ${estimate.inputTokens}`);
    ...
    console.log("[Sidepanel] Summary result:", result);
    ...
    console.error("[Sidepanel] Error summarizing messages:", error);
    loadingSection.style.display = "none";
...
async function handleExpandContext() {
  ...
  console.log(`[Sidepanel] Expanding context from ${currentState.messageCount} to ${newCount} messages`);
...
async function initializePopup() {
  ...
  // Advanced options toggle and restore/save for summaryLength
  const advToggle = document.getElementById("advanced-toggle");
  const advPanel = document.getElementById("advanced-panel");
  if (advToggle && advPanel) {
    advToggle.addEventListener("click", () => {
      advPanel.style.display =
        advPanel.style.display === "none" ? "block" : "none";
    });
  }

  // Restore summaryLength from storage
  chrome.storage.sync.get({ summaryLength: "standard" }, (items) => {
    const val = items.summaryLength || "standard";
    const radio = document.querySelector(
      `input[name="summaryLength"][value="${val}"]`
    );
    if (radio) radio.checked = true;
  });

  // Save when user changes the summary length in the sidepanel
  document.querySelectorAll('input[name="summaryLength"]').forEach((radio) => {
    radio.addEventListener("change", (e) => {
      try {
        chrome.storage.sync.set({ summaryLength: e.target.value });
      } catch (err) {
        console.error("Failed to save summaryLength:", err);
      }
    });
  });
  ...
  if (includeMediaCheckbox) {
    chrome.storage.sync.get({ includeMedia: false }, (items) => {
      includeMediaCheckbox.checked = items.includeMedia;
    });

    includeMediaCheckbox.addEventListener("change", (e) => {
      chrome.storage.sync.set({ includeMedia: e.target.checked });
    });
  }
